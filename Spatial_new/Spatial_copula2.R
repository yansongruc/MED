...
ResultC2_2= foreach(i=1:200,
                   .combine=cbind,
                   .packages=c("geoR","mined","np","MaxPro","nabor","mvtnorm")) %dopar% PEnonug_main(666*i+888,2)
stopCluster(cl)

MED_c22_pa=matrix(0,5,200)
MAP_c22_pa=matrix(0,5,200)
ABS_c22_pa=matrix(0,5,200)
UNI_c22_pa=matrix(0,5,200)
for(i in 1:5){
  for(j in 1:200){
    MED_c22_pa[i,j]=ResultC2_2[i,j][[1]][1,5]
    MAP_c22_pa[i,j]=ResultC2_2[i,j][[1]][2,5]
    ABS_c22_pa[i,j]=ResultC2_2[i,j][[1]][3,5]
    UNI_c22_pa[i,j]=ResultC2_2[i,j][[1]][4,5]
  }
}

data_C22=data.frame(mse=c(apply(MED_c22_pa,1,mean),apply(MAP_c22_pa,1,mean),apply(ABS_c22_pa,1,mean),apply(UNI_c22_pa,1,mean)),
                   sd=c(apply(MED_c22_pa,1,sd), apply(MAP_c22_pa,1,sd),apply(ABS_c22_pa,1,sd),apply(UNI_c22_pa,1,sd)),
                   Method=factor(rep(c("GBMED","SBS","ABS","UNIF"),each=5)),
                   k=rep(c(30,50,100,150,200),times=4))

pd=position_dodge(5)
p_C22=ggplot(data_C22,aes(x=k,y=mse,group=Method,colour=Method))+
