...
p2<-p +theme_bw()+
  ...
  geom_point(data=X[unif.ind,],aes(V1,V2),size=4,col="red")
p2
...
p3<-p +theme_bw()+
  ...
  geom_point(data=X[abs.ind,],aes(V1,V2),size=4,col="red")
p3
...
p4<-p +theme_bw()+
  ...
  geom_point(data=X[lhd2.ind,],aes(V1,V2),size=4,col="red")
p4
...
p5<-p +theme_bw()+
  ...
  geom_point(data=X[med.ind,],aes(V1,V2),size=4,col="red")
p5
...
# residual
df2=df[,1:2]
df2$rsbs=abs(df$y-df$sbsy)
df2$rmed=abs(df$y-df$medy)

#SBS
p <- ggplot(df2, aes(V1,V2))+geom_raster(aes(fill=rsbs))
p6<-p +theme_bw()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(colour = "black"),
...
p7<-p +theme_bw()+
  ...
  scale_fill_gradient(low = c1, high = c2)+
  xlab(expression(x[1]))+ylab(expression(x[2]))+
  geom_raster(data=df2,aes(fill=rmed))+
  geom_point(data=X[med.ind,],aes(V1,V2),size=4,col="red")
p7
